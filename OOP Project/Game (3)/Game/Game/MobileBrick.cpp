...
void MobileBrick::move(List* worldEnt)//instead of passing the entire list, pass two entities, prev and for bricks
{
    ...
    else if (dx==LEFT)
    {
        ...
        {
            if (prevBrick == NULL || prevBrick->getPos().x + prevBrick->getWidth() + 2 < this->pos.x || prevBrick->getPos().y < this->pos.y) //allowing brick movement only if the next brick doesn't collide, change direction otherwise
                pos.x -= speed;
            else
                dx=RIGHT;
        }
