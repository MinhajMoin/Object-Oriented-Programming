...
//            cout << "Output Char: " << outputChar << endl;
            if(outputChar=='1')
            {
                b=new OneHitBrick(gameBoard->gSpriteSheetTexture,spawnPoint);
                spawnPoint++;
            }
            else if (outputChar=='2')
            {
                b=new TwoHitBrick(gameBoard->gSpriteSheetTexture,spawnPoint);
                spawnPoint++;
            }
            else if (outputChar=='3')
            {
                b=new ThreeHitBrick(gameBoard->gSpriteSheetTexture,spawnPoint);
                spawnPoint++;
            }
            else if (outputChar=='s')
            {
                b=new SteelBrick(gameBoard->gSpriteSheetTexture,spawnPoint);
                spawnPoint++;
            }
            else if (outputChar=='\n')// || outputChar=='\n')
            {
