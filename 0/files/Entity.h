...
public:
    ...
    friend class Game;
    virtual int incrementDmg() {};//Here to support polymorphic call for bricks
    ...
    virtual void setPower(Entity*, int = 0, int = 0) {};
    //friend class FileIO;
    int getType();
    Point getPos();
    void setBounds();
    ...
    virtual void reset() {}; //will be redefined in ball,paddle, basically resets state (position & mode)
    enum ENTITIES {ONEHITBRICK,TWOHITBRICK,THREEHITBRICK,STEELBRICK,MOBILEBRICKX,MOBILEBRICKY,INVBRICK,PADDLE,BALL,POWERUP,BULLET};
protected:
    Point pos;
    SDL_Rect bounds;
    bool alive;
