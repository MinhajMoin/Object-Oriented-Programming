            ...
            while( !quit )
            {
                ...
                while( SDL_PollEvent( &e ) != 0 )
                {
                    ...
                    if (e.type == SDL_KEYDOWN)
                    {
                        ...
                        {
                        case (SDLK_p): // User presses P, then go into Point drawing mode
                            if (mode!=POINT)
                            {
                                mode = POINT;
                                cout << "Point Draw Mode" << endl;
                            }
                            break;

                        case (SDLK_l):// User presses L, then go into Line drawing mode
                            if (mode!=LINE)
                            {
                                mode = LINE;
                                cout << "Line Draw Mode" << endl;
                            }
                            break;

                        case (SDLK_r):// User presses R, then go into Rectangle drawing mode
                            if (mode!=RECT)
                            {
                                mode = RECT;
                                cout << "Rectangle Draw Mode" << endl;
                            }
                            break;
                        ...
                        case (SDLK_e): //User wants to redo something he did in the past.Assigned to E as an extra functionality
                            ...
                            if (poppedshapes->length()>0)
                            {
                                ...
                                if (pop->type == RECT)
                                {
                                    redoshape = new Rect(&(pop->fillRect),pop->colors,pop->ID);
                                }
                                if (pop->type == LINE)
                                {
                                ...
                                if (pop->type == POINT)
                                {
                                    redoshape = new Point(&pop->fillRect,pop->colors,pop->ID);
                                }
                    ...
                    if( e.type == SDL_MOUSEMOTION || e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEBUTTONUP )
                    {
                        ...
                        if(e.type == SDL_MOUSEBUTTONUP) //When user finishes clicking the mouse button
                        {
                            ...
                            if (e.button.button == SDL_BUTTON_LEFT)
                            {
                                ...
                                {
                                case (RECT):
                                    shape = new Rect(&fillRect,colors,ID);
                                ...
                                case (POINT):
                                {
                                    shape = new Point(&fillRect,colors,ID);
                                    ShapeList->append(shape);
